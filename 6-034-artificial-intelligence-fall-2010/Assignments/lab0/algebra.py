...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    if isinstance(expr1, Product) and len(expr1) == 1 and expr1[0] == 1:
        return expr2
    if isinstance(expr2, Product) and len(expr2) == 1 and expr2[0] == 1:
        return expr1
    ...
    if isinstance(expr1, Sum):
        if isinstance(expr2, Sum):
            ...
            # --- fun with python ---
            # appender = lambda x: [res.append(Product([el1, x]).flatten()) for el1 in expr1]
            # [appender(el2) for el2 in expr2]

            for el1 in expr1:
                res += [Product([el1, el2]).flatten() for el2 in expr2]

            return Sum(res)
        else:
            return Sum([Product([el1, expr2]).flatten() for el1 in expr1])
    else:
        if isinstance(expr2, Sum):
