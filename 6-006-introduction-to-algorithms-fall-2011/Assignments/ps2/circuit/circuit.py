...
class PriorityQueue:
    ...
    def __heapify(self, p):
        ...
        if left and self.__value(p) > self.__value(left):
            smallest = left

        if right and self.__value(smallest) > self.__value(right): 
            smallest = right

        if smallest != p:
            self.__swap(smallest, p)
            self.__heapify(smallest)

    def __build_min_heap(self):
        for h in reversed(range(len(self) // 2)):
            self.__heapify(h)
    ...
    def append(self, elem):
        ...
        idx = len(self) - 1
        while idx > 0 and self.__value(idx) < self.__value(self.__parent(idx)):
